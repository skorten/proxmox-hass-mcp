...
| Path | Responsibility |
| --- | --- |
| `ct/hass-mcp.sh` | Defines LXC defaults, collects application settings on the Proxmox host, exports them, invokes Community Scripts Core, and implements updates. |
| `install/hass-mcp-install.sh` | Runs non-interactively inside the new LXC, installs Python and Hass-MCP, writes configuration, and creates the systemd unit. |
...
2. Receives the Home Assistant URL, token, and MCP port collected by the host-side wizard, then validates the required token and port.
